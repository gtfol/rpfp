        ...
        <CardFooter className='flex justify-center'>
          {title.includes('Free') ? (
            userDetails.paid ? (
              ...
              <Button disabled={true}>{loading ? 'Please wait' : 'Current Plan'}</Button>
            )
          ) : title.includes('Pro') ? (
            userDetails.paid ? (
              <Button disabled={true}>{loading ? 'Please wait' : 'Current Plan'}</Button>
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className='max-w-2xl'>
        <DialogHeader>
          ...
          <DialogDescription>Please select a payment plan below.</DialogDescription>
        </DialogHeader>
